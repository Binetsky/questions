...
```
GET - /api/survey/[id] Получение опроса. Модель данных в директории src/models (В заголовке передать accessKey)

POST - /api/results Отправка результатов. Модель данных в директории src/models (в заголовке передать accessKey)

```

### Методы для работы внутри приложения

```
GET/POST - /api/admin/surveys Получение и запись опроса. Модель данных в директории src/models

GET - /api/admin/results Получение результатов. Модель данных в директории src/models
